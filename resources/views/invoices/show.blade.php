...
@section('action-button')
    ...
    @if($invoice->client_id == Auth::user()->id)
        @if($invoice->getDue() > 0)
            @if((isset($paymentSetting['enable_stripe']) && $paymentSetting['enable_stripe'] == 'on') || (isset($paymentSetting['enable_paypal']) && $paymentSetting['enable_paypal'] == 'on') || (isset($paymentSetting['is_paystack_enabled']) && $paymentSetting['is_paystack_enabled'] == 'on') || (isset($paymentSetting['is_flutterwave_enabled']) && $paymentSetting['is_flutterwave_enabled'] == 'on') || (isset($paymentSetting['is_razorpay_enabled']) &&
    ...
    <div class="card card-body p-md-5">
        <div class="row align-items-center mb-5">
            ...
            <div class="col-sm-6 text-sm-right">
                <span class="badge badge-pill badge-{{__(\App\Models\Invoice::$status_color[$invoice->status])}} ml-3">{{__(\App\Models\Invoice::$status[$invoice->status])}}</span>
            </div>
        </div>
        <div class="row mb-3">
            ...
            <div class="col-lg-6 col-md-6 text-right" >
                ...
                <p class="text-sm font-weight-700 mb-0">
                    ...
                    <div>
                        <div class="col-lg-6 col-md-6 text-align-right qrcodeinvoice" style="right: -356px; padding-top: 10px;" style="position: relative">
                    ...
                    <table class="table mb-0">
                        ...
                        @foreach($invoice->items as $item)
                            <tr>
                                ...
                                <td class="px-0">{{ \App\Models\Utility::projectCurrencyFormat($invoice->project_id,$item->price,true) }}</td>
                        ...
                        <div class="row justify-content-between align-items-center">
                            <div class="col-md-6 order-md-2 mb-4 mb-md-0">
                                <div class="d-flex align-items-center justify-content-md-end">
                                    ...
                                    <span class="h4 mb-0">{{ \App\Models\Utility::projectCurrencyFormat($invoice->project_id,($invoice->getSubTotal()+$invoice->getTax()),true) }}</span>
                                </div>
                            </div>
                            <div class="col-md-3 order-md-1">
                                <div class="text text-sm">
                                    <span class="font-weight-bold">{{__('Subtotal')}}</span><br>
                                    <span class="text-dark">{{ \App\Models\Utility::projectCurrencyFormat($invoice->project_id,$invoice->getSubTotal(),true) }}</span>
                                </div>
                            </div>
                            <div class="col-md-3 order-md-1">
                                <div class="text text-sm">
                                    <span class="font-weight-bold">{{(!empty($invoice->tax)?$invoice->tax->name:'Tax')}} ({{(!empty($invoice->tax)?$invoice->tax->rate:'0')}} %)</span><br>
                                    <span class="text-dark">{{ \App\Models\Utility::projectCurrencyFormat($invoice->project_id,$invoice->getTax(),true) }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        if (res.flag == 1) {
                            ...
                            var handler = PaystackPop.setup({
                                ...
                                callback: function (response) {
                                    ...
                                    window.location.href = paystack_callback + '/' + response.reference + '/' + '{{encrypt($invoice->id)}}?invoice_creator=' + res.invoice_creator;
                                },
                                onClose: function () {
                                    alert('window closed');
                                }
                            ...
                            var x = getpaidSetup({
                                ...
                                callback: function (response) {
                                    ...
                                    if (
                                        ...
                                        window.location.href = flutter_callback + '/' + txref + '/' + '{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}' + '?invoice_creator=' + res.invoice_creator;
                                    } else {
                        ...
                        if (res.flag == 1) {
                            ...
                            var options = {
                                ...
                                "handler": function (response) {
                                    window.location.href = razorPay_callback + '/' + response.razorpay_payment_id + '/' + '{{\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)}}?coupon_id=' + coupon_id + '&payment_frequency=' + res.payment_frequency + '&invoice_creator=' + res.invoice_creator;
                                },
                                "theme": {
                                    "color": "#528FF0"
                                }
